...
public final class SumArrayThreads {
  ...
  public static void main(final String[] args) {
    ...
    // NOTE: Parse the number of threads from the command-line arguments
    int numThreads = Integer.parseInt(args[0]);

    // Used for generating random array
    int[] arr = generateRandomArray(ARR_LEN);

    // NOTE: Set up a ThreadPoolExecutor with numThreads
    ExecutorService executor = Executors.newFixedThreadPool(numThreads);

    // Used for measuring execution time
    long startTime = System.currentTimeMillis();

    // NOTE: Use "computeParallelSum" method
    long sum = computeParallelSum(arr, executor, numThreads);
